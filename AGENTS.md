...
**Production (recommended on Vercel):** set `VITE_CLERK_PUBLISHABLE_KEY` and `CLERK_PUBLISHABLE_KEY` to matching **`pk_live_` / `sk_live_`** (never put `sk_` in `VITE_CLERK_PUBLISHABLE_KEY`). Leave `VITE_CLERK_PROXY_URL` **empty** (do not set `none`) so production uses `https://www.anima-protocol.com/api/__clerk/` (trailing slash; must match Clerk dashboard Proxy URL). Apex `anima-protocol.com` redirects to `www` in `vercel.json`. Verify `curl https://www.anima-protocol.com/api/healthz` and `curl https://www.anima-protocol.com/api/__clerk/v1/environment` return 200 after deploy. Redeploy **without build cache**. If sign-in stalls, the app auto-retries once without the proxy (direct custom FAPI).
